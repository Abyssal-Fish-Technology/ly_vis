...
// 设备操作菜单
export const DeviceOpMenu = inject('configStore')(
    observer(function DeviceOpMenu({ configStore }) {
        ...
        const menuList = useMemo(() => {
            ...
            const calculateConfig = params => {
                const { api, data, openModalFun, dataKey, disabled } = params
                return [
                    {
                        type: 'menu',
                        icon: <PlusOutlined />,
            ...
            const resultArr = [
                {
                    ...
                    click: () => {
                        ...
                        if (resultParams) {
                            Object.entries(
                                Object.prototype.toString.call(resultParams) ===
                                    '[object Function]'
                                    ? resultParams()
                                    : resultParams
                            ).forEach(d => {
        ...
        const basinInfoHandle = useCallback(() => {
            return new Promise(resolve => {
                getDeviceInfo(resultDevice).then(res => {
                    const { asset, geo, system, threat, type, info = [] } = res
                            ...
                            <>
                                {isBlack || isWhite || assetDesc.length ? (
                                    <>
                                        ...
                                        {isWhite && (
                                            <TagAttribute>白名单</TagAttribute>
                                        )}
                                        {assetDesc.map(d => (
                                            <TagAttribute type='asset' key={d}>
                                                {d}
                                            </TagAttribute>
                                        ))}
                                    </>
                                ...
                                <>
                                    {lastestSource ? (
                                        <TagAttribute type='asset'>
                                            {translateTiSource(lastestSource)}
                                        </TagAttribute>
                                    ) : (
                                        '--'
                                    )}
